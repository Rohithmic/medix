...
export default function MedicineDetails() {
      ...
      try {
        ...
        if (data?.category) {
          const relatedRes = await fetch(
            `/api/medicines?category=${data.category}`
          );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-white">
      ...
      {/* Product Section */}
      <div className="max-w-6xl mx-auto px-6 py-12 mt-8 flex flex-col md:flex-row gap-10 bg-white rounded-lg shadow-lg border border-gray-200">
        {/* Left: Product Image with fixed height */}
        <div className="flex flex-col items-center gap-4">
          <div className="w-[350px] h-[350px] bg-gray-50 flex items-center justify-center border rounded-lg overflow-hidden">
            <Image
              src={selectedImage}
              alt={medicine.name}
              width={350}
          ...
          <div className="flex gap-2 flex-wrap justify-center">
            {medicine.image?.map((img, idx) => (
              <div
                key={idx}
                className={`w-16 h-16 border rounded-lg cursor-pointer overflow-hidden flex items-center justify-center ${
        ...
        <div className="flex-1">
          ...
          {/* Price Box */}
          <div className="mt-6 bg-blue-50 p-4 rounded-lg inline-block shadow-sm">
        ...
        {related.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {related.map((med) => (
              ...
              >
                <div className="w-full h-40 bg-blue-50 flex justify-center items-center p-4 overflow-hidden">
                  <Image
